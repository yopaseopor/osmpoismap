...
style: function (feature) {
...
},
		{
			...
			title: 'Número portal',
			query: '(nwr["addr:housenumber"]({{bbox}});node(w);nwr["historic"="lavoir"]({{bbox}});node(w););out meta;',
			iconSrc: imgSrc + 'icones/maxspeed_empty.svg',
			iconStyle: 'background-color:rgba(255,255,255,0.4)',
style: function (feature) {
...
style: function (feature) {
	...
	onClickEvent: function(evt, view, coordinateLL) {
		...
		show.append($('<a>').css('marginLeft', 5).attr({title: 'OpenStreetBrowser', href: 'https://www.openstreetbrowser.org/#map=' + Math.min(view.getZoom(), 20) + '/' + coordinateLL[1] + '/' + coordinateLL[0] , target: '_blank'}).html($('<img>').attr({src: imgSrc + 'osb_logo.png', height: 20, width: 20})));
				...
				//Tracesmap
		show.append($('<a>').css('marginLeft', 5).attr({title: 'Tracesmap', href: 'https://www.tracesmap.com/#' + Math.min(view.getZoom(), 20) + '/' + coordinateLL[1] + '/' + coordinateLL[0] , target: '_blank'}).html($('<img>').attr({src: imgSrc + 'tracesmap.png', height: 20, width: 20})));
